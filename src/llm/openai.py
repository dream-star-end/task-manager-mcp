...
class OpenAILLM(LLMInterface):
    ...
    def __init__(self, api_key: Optional[str] = None, config: Optional[Dict[str, Any]] = None):
        ...
        if not self.api_key:
            logger.error("OpenAI API Key not provided or found in environment variable OPENAI_API_KEY.")
            raise ValueError("OpenAI API Key is required.")

        try:
            # Create both sync and async clients for flexibility
            self.client = OpenAI(api_key=self.api_key)
            self.async_client = AsyncOpenAI(api_key=self.api_key)
            logger.info("OpenAI SDK configured successfully.")
        except Exception as e:
            logger.error(f"Failed to configure OpenAI SDK: {e}", exc_info=True)
            raise RuntimeError(f"Failed to configure OpenAI SDK: {e}") from e

        # Allow overriding the model via config, default to GPT-4
    ...
    async def generate_text_async(
        ...
        **kwargs: Any
        ...
        """
        logger.debug(f"Generating text with OpenAI (model: {self.model_name}, temp: {temperature}, max_tokens: {max_tokens})")
        ...
        try:
            ...
            if response.choices and len(response.choices) > 0:
                ...
                if generated_text is not None:
                    logger.debug("OpenAI text generation successful.")
                    return generated_text
                else:
                    logger.warning("OpenAI response contained empty content.")
                    return ""
            else:
    ...
    async def generate_structured_content_async(
        ...
        **kwargs: Any
        ...
        """
        logger.debug(f"Generating structured content with OpenAI (model: {self.model_name}, temp: {temperature})")

        if not isinstance(schema, dict):
             raise ValueError("Schema must be a dictionary.")

        try:
            # Use the OpenAI Chat API with function calling capabilities
            response = await self.async_client.chat.completions.create(
                model=self.model_name,
                messages=[
                    {"role": "user", "content": prompt}
            ...
            if not response.choices or len(response.choices) == 0:
                logger.warning(f"OpenAI structured response for prompt '{prompt[:50]}...' contained no choices.")
                raise Exception("OpenAI returned an empty response for structured content.")
            ...
            logger.error(f"Failed to decode JSON response from OpenAI: {e}. Response text: {json_string if 'json_string' in locals() else 'N/A'}", exc_info=True)
            raise Exception(f"Failed to parse LLM JSON response: {e}") from e
        except openai.OpenAIError as e:
            logger.error(f"OpenAI API error during structured generation: {e}", exc_info=True)
            raise Exception(f"OpenAI API error: {e}") from e
        except Exception as e:
            # Catch potential issues if the response structure isn't as expected
            logger.error(f"Unexpected error during OpenAI structured generation: {e}", exc_info=True)
    ...
    async def parse_prd_to_tasks_async(
        ...
        **kwargs: Any
        ...
        """
        logger.info(f"Parsing PRD content using OpenAI model: {self.model_name}")
        ...
        prompt = f"""你是一个专业的产品需求文档解析助手。请仔细分析以下 PRD 文档，并提取 **顶级主任务**。
...
-----------------"""
        ...
        try:
            ...
            if not isinstance(parsed_data, list) or not all(isinstance(item, dict) for item in parsed_data):
                logger.error(f"OpenAI returned unexpected structure for PRD parsing. Expected List[Dict], got {type(parsed_data)}.")
            ...
            logger.info(f"Successfully parsed PRD into {len(validated_tasks)} validated task dictionaries using OpenAI.")
            return validated_tasks # Return only validated tasks

        except Exception as e:
            logger.error(f"Error during OpenAI PRD parsing with hierarchical IDs: {e}", exc_info=True)
            raise
    ...
    async def generate_subtasks_for_task_async(
        ...
        **kwargs: Any
        ...
        logger.info(f"Generating subtasks for task {task_id} ({task_name}) using OpenAI model")
...
"""
        ...
        try:
            ...
            if not isinstance(subtasks_data, list):
                logger.error(f"OpenAI returned unexpected structure for subtask generation. Expected list, got {type(subtasks_data)}")
            ...
            logger.error(f"Error generating subtasks with OpenAI: {e}", exc_info=True)
