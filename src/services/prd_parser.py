...
class PrdParser:
    ...
    async def parse_with_llm(self, prd_content: str) -> List[Task]:
        ...
        # 确保在解析之前清空任务存储，避免ID冲突
        try:
            logger.info("[PrdParser LLM] 清空任务存储以避免ID冲突...")
            self.storage.clear_all_tasks()
            logger.info("[PrdParser LLM] 任务存储已清空")
        except Exception as clear_e:
            logger.error(f"[PrdParser LLM] 清空任务存储失败: {clear_e}")
            logger.warning("[PrdParser LLM] 将继续解析，但可能存在ID冲突风险")
        ...
        try:
            ...
            try:
                # 获取所有现有任务（主任务+子任务）
                all_tasks, _ = self.storage.list_tasks()
                existing_task_ids = {task.id for task in all_tasks}
                logger.debug(f"[PrdParser LLM] 存在的任务ID: {existing_task_ids}")
            except Exception as e:
                logger.warning(f"[PrdParser LLM] 获取现有任务列表时出错: {e}")
            ...
            for task_info in tasks_data:
                 ...
                 original_llm_task_id = str(task_info['id']) # 原始 LLM 生成的 ID
                 llm_task_id = original_llm_task_id # 初始化为原始 ID，如果有冲突会被修改
                 ...
                 # 如果该ID已存在，尝试生成新的非冲突ID
                 if llm_task_id in existing_task_ids:
                     # 如果是简单数字ID（例如 1, 2, 3），则尝试生成新的顺序ID
                     if llm_task_id.isdigit():
                         # 查找当前最大数字ID，并生成一个新的
                         max_digit_id = 0
                         for task_id in existing_task_ids:
                             if task_id.isdigit() and int(task_id) > max_digit_id:
                                 max_digit_id = int(task_id)
                         
                         # 使用递增的数字ID
                         llm_task_id = str(max_digit_id + 1)
                         logger.info(f"[PrdParser LLM] ID 冲突: 原始ID '{original_llm_task_id}' 已存在，生成新ID '{llm_task_id}'")
                     
                     # 如果是分层ID（例如 1.1, 2.3），尝试生成相同级别的新ID
                     elif '.' in llm_task_id:
                         # 解析前缀和后缀
                         prefix, suffix = llm_task_id.rsplit('.', 1)
                         ...
                         if prefix not in id_counters:
                             ...
                             max_suffix = 0
                             for task_id in existing_task_ids:
                                 if task_id.startswith(f"{prefix}.") and task_id[len(prefix)+1:].isdigit():
                                     current_suffix = int(task_id[len(prefix)+1:])
                                     if current_suffix > max_suffix:
                                         max_suffix = current_suffix
                             id_counters[prefix] = max_suffix
                         
                         # 增加计数器并使用新的后缀
                         id_counters[prefix] += 1
                         llm_task_id = f"{prefix}.{id_counters[prefix]}"
                         logger.info(f"[PrdParser LLM] ID 冲突: 原始ID '{original_llm_task_id}' 已存在，生成新ID '{llm_task_id}'")
                     ...
                     # 如果是其他格式的ID，添加唯一后缀
                         ...
                         llm_task_id = f"{original_llm_task_id}_{unique_suffix}"
                         logger.info(f"[PrdParser LLM] ID 冲突: 原始ID '{original_llm_task_id}' 已存在，生成新ID '{llm_task_id}'")

                 logger.debug(f"[PrdParser LLM] Attempting to create task with ID: {llm_task_id}, Name: {task_name}")
                 try:
                     # ----> Pass the potentially modified ID to create_task <----
                    task = self.storage.create_task(
                        id=llm_task_id, # 使用可能修改过的ID
                        name=task_name,
                    ...
                    # 记录LLM生成的原始ID与实际使用的ID的映射
                    llm_id_to_task_id[original_llm_task_id] = task.id 
                    # 将新创建的任务ID添加到已存在ID集合中
                    ...
                    task_id_map[task_name] = task.id 
                      ...
                      logger.error(f"[PrdParser LLM] Failed to create task with ID {llm_task_id} for name '{task_name}' (ID might already exist): {ve}", exc_info=False) # Don't need full traceback for expected error
                      # Map LLM ID to None to indicate failure
                      llm_id_to_task_id[original_llm_task_id] = None 
                 except Exception as create_exc:
                      logger.error(f"[PrdParser LLM] Failed to create task with ID {llm_task_id} for name '{task_name}': {create_exc}", exc_info=True)
                      # Map LLM ID to None
                      llm_id_to_task_id[original_llm_task_id] = None 
                      continue
